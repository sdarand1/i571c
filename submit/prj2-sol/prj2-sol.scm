...
(define (dept-employees-names dept employees)
  (if (null? employees)
      '()
      (if (equal? (list-ref (car employees) 2) dept)
         (append '() (cons (caar employees) (dept-employees-names dept (cdr employees))))
...
(define (list-access indexes list)
  (if (> (length indexes) 0)
      (if (and (pair? list) (= (length indexes) 1))
          (if (< (car indexes) (length list))
...
(define (employees-salary-sum employees)
  (if (not (pair? employees))
      0
      (+ (list-ref (car employees) 3) (employees-salary-sum (cdr employees)))
  )
)
...
(define (dept-employees-names-salaries dept employees)
  (map (lambda (e) (list (list-ref e 0) (list-ref e 3))) (filter (lambda (e) (equal? (caddr e) dept)) employees))
)
...
(define (employees-average-salary employees)
  (if (not (eq? (length employees) 0))
      (/ (foldl + 0 (map (lambda (e) (list-ref e 3)) employees)) (length employees))
      0
  )
)
...
(define (int-list-json int-list)
  (if (list? int-list)
      (string-append "[" (string-join (map (lambda (l) (if (list? l) (int-list-json l) (number->string l))) int-list) ",") "]")
