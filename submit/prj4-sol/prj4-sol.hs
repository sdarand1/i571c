...
comprDeptEmployees emps dept = [ emp | emp <- emps, emplDept emp == dept]
...
employeesSalarySum emps = foldl (+) 0 (map emplSalary emps)
...
expn x = zipWith (/) (map (x**) infinite) (map fact infinite) 
    where
        fact 0 = 1
        fact n = n * fact (n-1)
...
charIndexes str chr = map snd (filter(\p -> fst p == chr) (zip str [0..(length str)]))
...
foldTree tF lF (Tree lT n rT) = tF (foldTree tF lF lT) n (foldTree tF lF rT)
foldTree tF lF (Leaf n) = lF n
...
flattenTree tree = foldTree(fn1) (fn2) tree
  where fn1 tr1 tr tr2 = (tr1 ++ [tr] ++ tr2)
        fn2 tr = [tr]
...
catenateTreeLists t = foldl (++) [] (flattenTree t)
